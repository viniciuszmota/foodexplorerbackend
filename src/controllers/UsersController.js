...
class UsersController {
  async create(request, response) {
    ...
    if (!name || !email || !password)
      throw new AppError("Nome, email e password são obrigatórios", 401)

    if (!isEmailValid(email)) throw new AppError("E-mail inválido!", 401)

    if (password.length < 6)
      throw new AppError("Senha muito curta! Mínimo de 6 caracteres.", 401)
  ...
  async update(request, response) {
    ...
    if (newPassword) {
      if (!password)
        throw new AppError(
          "Informe a senha atual para cadastrar uma nova senha!",
          404
        )
