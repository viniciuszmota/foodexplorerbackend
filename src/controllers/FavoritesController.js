...
class FavoritesController {
  ...
  async create(request, response) {
    ...
    const favoriteExists = await knex("favorites_dishes")
    ...
    const [favorited] = await knex("favorites_dishes").insert({
      dish_id,
      user_id,
    })

    if (favorited) return response.status(201).end()

    throw new AppError("Erro ao registrar o favorito!")
  }
  ...
  async delete(request, response) {
    ...
    const hasDeleted = await knex("favorites_dishes")
      .where({ dish_id, user_id })
      .delete()
